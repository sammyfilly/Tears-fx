...
describe("ProgressHandler", () => {
  ...
  it("output", async () => {
    ...
    let expected =
      "test title: [0/1] Prepare task. Check [output window](command:ms-copilot-extension.showOutputChannel) for details. (Notice: You can reload the window and retry if task spends too long time.)";
    await progressHandler.start();
    chai.assert.equal(message, expected);
    await progressHandler.next("test message.");
    expected =
      "test title: [1/1] test message. Check [output window](command:ms-copilot-extension.showOutputChannel) for details. (Notice: You can reload the window and retry if task spends too long time.)";
    chai.assert.equal(message, expected);
    sinon.restore();
  });
