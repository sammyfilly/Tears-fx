...
describe("handlers", () => {
  ...
  describe("command handlers", function () {
    ...
    it("selectTutorialsHandler()", async () => {
      ...
      chai.assert.isTrue(result.isOk());
      chai.assert.equal(tutorialOptions[1].data, "https://aka.ms/ms-copilot-notification-new");
    });
    ...
    it("selectTutorialsHandler() for SPFx projects - v3", async () => {
      ...
      chai.assert.isTrue(result.isOk());
      chai.assert.equal(tutorialOptions[0].data, "https://aka.ms/ms-copilot-add-cicd-new");
    });
  });
