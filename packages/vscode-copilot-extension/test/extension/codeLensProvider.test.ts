...
describe("Manifest codelens", () => {
  ...
  it("ResolveEnvironmentVariableCodelens", async () => {
    ...
    chai.assert.equal(res.command?.command, "ms-copilot-extension.openConfigState");
  ...
  it("ComputeTemplateCodeLenses for AAD manifest", async () => {
    ...
    chai.assert.isTrue(
      res != null && res[0].command!.command === "ms-copilot-extension.openPreviewAadFile"
    );
  });
});
...
describe("Crypto CodeLensProvider", () => {
  ...
  it("envData codelens", async () => {
    ...
    chai.expect(codelens[0].command?.command).equal("ms-copilot-extension.decryptSecret");
    sinon.restore();
  });
