...
export class ManifestTemplateHoverProvider implements vscode.HoverProvider {
  ...
  private async generateHoverMessageV3(
      ...
      for (const envName of envNames) {
        ...
        if (value) {
          ...
          if (envName === environmentManager.getLocalEnvName()) {
            ...
            const commandUri = vscode.Uri.parse("command:ms-copilot-extension.provision");
    ...
    const commandUri = vscode.Uri.parse(
      `command:ms-copilot-extension.openConfigState?${encodeURIComponent(JSON.stringify(args))}`
    );
